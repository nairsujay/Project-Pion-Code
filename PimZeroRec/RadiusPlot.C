...
void ProcCmp()
{
    ...
    TH2D* ovflowhist = new TH2D("ovflowhist","Correlation between Energy Overflow and Process;Process;Overflow/Mev",8,0,8,40,0,20.0);
...
void ZeroPlot()
{
    ...
    TList* lout = new TList;
    TTree* tree = PlotHelp::InputFile("Pion_Output_Recur.root","tout");
    ...
    tree->Draw("secondary_proc>>+zerohist","(nrec<0)&&(truth_Trkr)");
    tree->Draw("secondary_proc>>+allhist","(truth_Trkr)");
    tree->Draw("secondary_proc>>+passhist","(nrec>=0)&&(truth_Trkr)");
