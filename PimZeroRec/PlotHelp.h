...
//const Color_t ColorArray[8] = {kBlack,kBlue,kRed,kGreen,kMagenta,kYellow,kCyan,kViolet};
const Color_t ColorArray[8] = {kBlack,kBlue,kGreen,kYellow,kMagenta,kRed,kCyan,kGray};
...
class PlotHelp
{
    public:
        ...
        static  void Plot1DModule(TTree *tt, TCanvas *c1, const TString tag, 
                     ...
                     TList *list, const TString outdir);
        ...
        Bool_t kZERO, Int_t ii)
{
    ...
    {
        hh->SetTitle(title+Form(" - Energy Deposited at Tracker Radius = %d",ii));
    }
      ...
      
{
    ...
    TString hname=tag+var;

    const Double_t maxh=1.1;
    TH1D *hh=new TH1D(hname,tag+"__"+var,nbins, xmin, xmax); 
    if(list){
        list->Add(hh);
    }
    ...
    hh->SetFillColor(kRed);
    hh->SetLineColor(kRed); hh->SetLineWidth(2); hh->Draw();
    c1->Print(Form("%s/%s_%s.png",outdir.Data(), tag.Data(),var.Data()));
