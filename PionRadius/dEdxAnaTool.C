...
TChain * dEdxAnaTool::InputFiles(const TString file, const TString tr)
{
    ...
    if(file.Contains(".root"))
        ...
        ifstream fin(file);
        if(!fin){
            printf("InputFiles file not found \n%s\n\n",file.Data()); exit(1);
        }
...
void dEdxAnaTool::Loop(const TString filelist)
{
    ...
    Double_t Eint        = 0.0;
    Double_t llrproton   = 0.0;
    Double_t llrpion     = 0.0;
    Int_t    llroffpion  = 0.0;
    Int_t    llroffproton= 0.0;

    Int_t radiusrange=200; //in 5 cm
    Double_t binsize=5.0; //Convert radius to bins 
    Int_t    Radiusbin[radiusrange];    
    Double_t Energybin[radiusrange];
    Int_t    radind      = 0.0;

    std::vector<double> Radiusunbin;
    std::vector<double> Energyunbin;
    std::vector<double> Zarray;
    std::vector<double> Tarray;

    Int_t arrsize = 0; //Number of clusters (equiv. to size of vector)

    Bool_t   truth_Trkr; //Bool for trkr events
    Double_t z0 = 0.0; //Track endpoint z
    Double_t t0 = 0.0; //Track endpoint x
    ...
    tout->Branch("trkr",&evis_trkr,"trkr/D");
    tout->Branch("trkr10",&evis_trkr10,"trkr10/D");
    tout->Branch("trkr15",&evis_trkr15,"trkr15/D");
    tout->Branch("trkr20",&evis_trkr20,"trkr20/D");
    tout->Branch("trkr25",&evis_trkr25,"trkr25/D");
    tout->Branch("trkr30",&evis_trkr30,"trkr30/D");
    tout->Branch("ecal",&evis_ecal,"ecal/D");
    tout->Branch("ecal10",&evis_ecal10,"ecal10/D");
    tout->Branch("ecal15",&evis_ecal15,"ecal15/D");
    tout->Branch("ecal20",&evis_ecal20,"ecal20/D");
    tout->Branch("ecal25",&evis_ecal25,"ecal25/D");
    tout->Branch("ecal30",&evis_ecal30,"ecal30/D");
    tout->Branch("ecalo",&ecalo,"ecalo/D");
    tout->Branch("E",&E_E,"Energy/D");
    tout->Branch("kinetic",&kinetic,"Kinetic/D");
    tout->Branch("Eint",&Eint,"Eint/D");
    tout->Branch("llrproton",&llrproton,"llrproton/D");
    tout->Branch("llrpion",&llrpion,"llrpion/D");
    tout->Branch("llroffpion",&llroffpion,"llroffpion/I");
    tout->Branch("llroffproton",&llroffproton,"llroffproton/I");


    tout->Branch("radiusrange",&radiusrange,"radiusrange/I");
    tout->Branch("binsize",&binsize,"binsize/D");
    tout->Branch("Radiusbin",&Radiusbin,"Radiusbin[radiusrange]/I");
    tout->Branch("Energybin", &Energybin,"Energybin[radiusrange]/D");


    tout->Branch("arrsize",&arrsize,"arrsize/I");
    tout->Branch("Radiusunbin",&Radiusunbin);
    tout->Branch("Energyunbin",&Energyunbin);
    tout->Branch("Zarray",&Zarray);
    tout->Branch("Tarray",&Tarray);



    tout->Branch("truth_Trkr",&truth_Trkr,"truth_Trkr/O");


    tout->Branch("z0",&z0,"z0/D");
    tout->Branch("t0",&t0,"t0/D");
    //-------------------------------------------------------
    ...
    for (Long64_t jentry=0; jentry<nentries;jentry++) {
        ...
        truth_Trkr = kTRUE; //Defaults to true and set as false if cluster is outside trkr
        for(Int_t ii=0;ii<radiusrange;ii++){Radiusbin[ii]=0;Energybin[ii]=0;}; //Initialize binned array of radii and energy
        ...
        llrproton = llr_score_proton; 
        llrpion   = llr_score_pion;

        llroffproton = llr_offset_proton;
        llroffpion   = llr_offset_pion;
        ...
        for (int i = 0; i < cluster_energy_vec_sz; ++i) {
            ...
            //Convert radius to bin number to fill Radiusbin
            radind = floor(radius/binsize);
            if(radind<radiusrange) 
            {
                Radiusbin[radind] += 1; //binned clusters
                Energybin[radind] += energy;
            }

            Radiusunbin.push_back(radius); //unbinned radii
            Energyunbin.push_back(energy);
            Zarray.push_back(z);
            Tarray.push_back(t);

            if(z > 8585.0) truth_Trkr = kFALSE;    
            //If a single cluster is outside trkr region event is deemed to not be in trkr
        ...
        ecalo = evis_trkr20/0.8 + evis_ecal20/0.45; //weighted sum of ecal and trkr used by Trung
        ...
        arrsize = Energyunbin.size();
        for(Int_t ii = 0;ii<arrsize;ii++){printf(" %g ",Energyunbin[ii]);};
        if(arrsize != Zarray.size()){fprintf(stderr,"\n\n\n \t FAILED \t \n\n\n");exit(1);}; //fail if vectors have different sizes 
