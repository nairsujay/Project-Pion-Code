...
const TString gcut("&&(llrpion<0)"); //gets rid of """false""" zeroes [[[STILL IN TESTING]]]
...
void ReproducePlot()
{
    ...
    for(Int_t ii=0;ii<8;ii++) //Nested loops iterate over process and radius
    {
        ...
        {        
            PlotHelp::NicePlot(tree,c1,Proc[ii],Form("trkr%d",jj),Cut[ii]+trkrecal,60,0,160,lout,"outplot/reproduce",kFALSE,jj);

            PlotHelp::NicePlot(tree,c1,Proc[ii],Form("ecal%d",jj),Cut[ii]+trkrecal,60,0,160,lout,"outplot/reproduce",kFALSE,jj);
        }
    }
...
//{
    ...
    //const TString ecalcut("&&(ecal10 != 0)");
    //const TString trkrcut("&&(trkr10 != 0)");

    //const TString origtrkrcut("&&(trkr10 + trkr15 + trkr20 + trkr25 + trkr30  !=0)");
    //const TString origecalcut("&&(ecal10 + ecal15 + ecal20 + ecal25 + ecal30  !=0)");
    ...
    //{
        //for(Int_t jj=10;jj<=30;jj+=5)
        //{
            //PlotHelp::Plot1DModule(tree,c1,Proc[ii],Form("trkr%d",jj),Cut[ii]+trkrcut,60,0,160,lout,"outplot/zerocut",kTRUE,jj);
...
//Actually Reproduces Trung's plots

void BothPlot()
{
    TList* lout = new TList ;

    const TString Proc[8] = {"Absorption","Multi-Production","ChargeExchange","DoubleChargeExchange","Inelastic","Rangeout","Decay","all"};
