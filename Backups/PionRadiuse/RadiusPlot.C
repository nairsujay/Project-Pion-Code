...
void ERCorr()
{
    ...
    TH2D* erhists[8];
    TH2D* unerhists[8];
    TH2D* othhist = new TH2D("othhist","Others",19,5,100,25,0,150);
    ...
    for(Long64_t jentry = 0;jentry<tree->GetEntries();jentry++)
    {
        ...
        {
            Esum = 0;
            for(Int_t ii = indmax-1 ;ii>=0;ii += -1)
            {
                Esum += Energybincut[ii];
                radius = (indmax-ii)*5.0; //in cm
                if(Energybincut[ii] != 0) othhist->Fill(radius,Energybincut[ii]);//Esum
            }
    ...
    {
        TH2D* tmph = new TH2D("tmph","",19,5,100,25,0,150);
        ...
        for(Long64_t jentry = 0; jentry<tree->GetEntries();jentry++)
        {
            ...
            if(secprocnum == iproc)
            {
                ...
                for(Int_t ii = indmax-1 ;ii>=0;ii += -1)
                {
                    Esum +=Energybincut[ii];
                    radius = (indmax-ii)*5.0; //in cm
                    if(Energybincut[ii] != 0) tmph->Fill(rmax-radius,Energybincut[ii]);
                }
...
//Loop over bins 
    ...
    {
        THStack* tmphs = 0x0;
        if(outname.Contains("tail")) tmphs =  new THStack(Form("radius_%g",jj*5.0),Form("Energy Deposited at radius %g",jj*5.0));
        if(outname.Contains("head")) tmphs = new THStack(Form("radius_%g",jj*5.0),Form("Energy Deposited at Radius %g",(jj+1)*5.0));
