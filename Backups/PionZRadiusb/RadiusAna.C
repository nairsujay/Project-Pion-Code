...
void Printer()
{
    ...
    {
        tree->GetEntry(ii);
        if((Radiusbin[0] == 0)&&(Radiusbin[1]==0)){printf("\n\nFailed\n\n");exit(1);}; //exit if filtering failed
...
void ZeroFilter()
{
    ...
    TTree* filteree =(TTree*)intree->CopyTree("(Radiusbin[0]+Radiusbin[1]>0)"+trkrcut); 
...
void RadAna()
{
    ...
    Int_t indmax, ovflow;
    Double_t Radiusbincut[100];
    Double_t Energybincut[100];
    ...
    TBranch *radcutbranch = tree->Branch("Radiusbincut",&Radiusbincut,"Radiusbincut[indmax]/D");
    TBranch *energycutbranch = tree->Branch("Energybincut",&Energybincut,"Energybincut[indmax]/D");
